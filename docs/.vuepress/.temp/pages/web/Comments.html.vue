<template><div><p>LearnHabs 使用的是 Waline 评论插件，部署简单，有后端管理。评论插件推荐次序为 Waline &gt; Valine &gt; giscus &gt; Gitalk &gt; Gitter。</p>
...
<p>Waline 支持 Akismet 反垃圾评论和免注册留言，但匿名留言会带来安全隐患。建议开启评论通知来避免出现极端情况。Waline 的评论通知支持多种方式，包括 QQ、微信、邮件等。对于评论的回复，仅支持邮件通知。</p>
<p>Vercel 项目中选择「Overview」&gt;「Settings」&gt;「Environment Variables」，然后按照 <a href="https://waline.js.org/guide/server/notification.html" target="_blank" rel="noopener noreferrer">Waline 评论通知<ExternalLinkIcon/></a>配置所需环境变量。如果需要 Webhook 等更多通知方式，则查看 <a href="https://waline.js.org/reference/server.html" target="_blank" rel="noopener noreferrer">Waline 服务端配置<ExternalLinkIcon/></a>。配置成功后，选择「Overview」&gt;「Deployments」，进行 Redeploy，重新部署后环境变量和评论通知方会生效。目前 <code v-pre>*.vercel.app</code> 域名已经被 DNS 污染，会出现「无法加载评论」，国内使用建议绑定自定义域名，域名对备案暂无硬性要求。</p>
<h3 id="手动部署" tabindex="-1"><a class="header-anchor" href="#手动部署" aria-hidden="true">#</a> 手动部署</h3>
<p>请注意，Vercel 和 LeanCloud 有额度限制。如果你的网站日访客超过一千人次，建议使用自行部署的方案。以下是我的部署步骤：</p>
<ol>
<li>
<p>新建数据库，并导入 <a href="https://github.com/walinejs/waline/blob/main/assets/waline.sql" target="_blank" rel="noopener noreferrer">waline.sql<ExternalLinkIcon/></a> 以完成表和表结构的创建，之后在项目中配置如下环境变量。<sup class="footnote-ref"><a href="#footnote1">[1]</a><a class="footnote-anchor" id="footnote-ref1" /></sup></p>
</li>
<li>
<p>配置必要的环境变量，可添加于 /etc/environment。配置完成后，需要重启项目让环境变量生效。并注意最后一行需保留空行。</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token assign-left variable">MYSQL_DB</span><span class="token operator">=</span><span class="token string">""</span>
...
<li>
<p>部署 Waline 服务端，使用 forever 让其持久化运行。</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token comment"># 切换到站点路径</span>
...
<li>
<p>打开网页管理后台，通过导入事先备份的 JSON 文件来还原数据。</p>
</li>
</ol>
<p>新手可以使用宝塔面板的 Node.js 版本管理器来部署 Node.js。安装后，受限更新版本列表，然后选择要使用的 Node 版本，将其设置为命令行版本，接着配置服务器的环境变量。虽然宝塔可以管理 Node 项目，但容易出现错误。处理流程为进入「网站」&gt;「Node 项目」&gt;「添加 Node 项目」，将启动选项设置为模块内的 vanilla.js 文件，将项目端口设置为 8360，并设置绑定域名。</p>
<p>注意：如果你选择手动部署反向代理服务器，请务必使用 Waline 官方提供的 Nginx 配置文件，否则可能会导致<strong>无法登录后台</strong>，或是<strong>版本升级后仍然提示升级</strong>等问题。如果仍然有问题，请检查是否使用 SSH 直连服务器，而非宝塔自带网页终端。宝塔网页终端执行的命令有几率出错，一定要使用正常的 SSH 链接。</p>
...
<span class="token punctuation">{</span>
  ...
  server_name waline.gpt-vip.top<span class="token punctuation">;</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="导入时间处理" tabindex="-1"><a class="header-anchor" href="#导入时间处理" aria-hidden="true">#</a> 导入时间处理</h2>
<p>如果 Waline 数据导入还原后，评论发布时间被重置为当前时间，则说明时间格式不兼容。以下以 LeanCloud 转 MySQL 为例：</p>
<ol>
<li>
<p><code v-pre>2023-03-24T17:27:28.121Z</code> 这个时间戳中的格式 &quot;T&quot; 和 &quot;Z&quot; 不被 MySQL 支持，要将 &quot;T&quot; 替换为空格并将 &quot;Z&quot; 删除。我写了个<a href="https://web-platform-dzhkey.stackblitz.io" target="_blank" rel="noopener noreferrer">网页工具<ExternalLinkIcon/></a>处理时间格式替换。</p>
</li>
<li>
<p>提取数据中的 Comment、Counter 和 Users，格式为 <code v-pre>[{},{}……,{}]</code>，依次复制到 <a href="https://www.convertjson.com/json-to-sql.htm#" target="_blank" rel="noopener noreferrer">json2sql<ExternalLinkIcon/></a> 进行操作。</p>
</li>
<li>
<p>在 json2sql 页面中，点击按钮「Format JSON」，然后按截图勾选「Still not happy」「Enclose field names」「Backtick (<code v-pre>name</code>)」，取消勾选第一个值的 key。同时，取消勾选 objectId 的「Include」，该项将不会导入数据库。</p>
<figure><img src="https://img.gpt-vip.top/2023-03-24-16-52-57.png?imageMogr2/format/webp" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
</li>
<li>
<p>将「Schema.Table or View Name」依次设为「`wl_Comment`」「`wl_Counter`」「`wl_Users`」，取消勾选「Create Table/View」。</p>
</li>
<li>
<p>在 Step 3: Generate output 下点击按钮「JSON TO SQL Insert」，然后点击复制按钮。</p>
</li>
<li>
<p>进入 phpMyAdmin 数据库后台，选中要操作的表（如 wl_Comment），点击右侧的 SQL 按钮运行 SQL 查询，将复制的内容粘贴到此，并点击执行。若无报错，则说明数据导入正常。</p>
</li>
</ol>
<p>处理后时间可以正常导入，但由于 id 结构不同，旧的评论回复将失去联系。</p>
...
<p><code v-pre>https://github.com/settings/developers</code> 页面管理已建的应用，获取 Client ID 和 Client secrets，修改应用链接。<sup class="footnote-ref"><a href="#footnote2">[2]</a><a class="footnote-anchor" id="footnote-ref2" /></sup></p>
<p>将以下代码复制粘贴至 docsify 页面，即可启动 Gitalk：</p>
...
<li id="footnote1" class="footnote-item"><p><a href="https://waline.js.org/guide/database.html#mysql" target="_blank" rel="noopener noreferrer">Waline 多数据库服务支持<ExternalLinkIcon/></a> <a href="#footnote-ref1" class="footnote-backref">↩︎</a></p>
</li>
<li id="footnote2" class="footnote-item"><p><a href="https://blog.csdn.net/qq_39052513/article/details/108291272" target="_blank" rel="noopener noreferrer">超 Nice 的评论组件 —— Gitalk<ExternalLinkIcon/></a> <a href="#footnote-ref2" class="footnote-backref">↩︎</a></p>
</li>
</ol>
</section>
</div></template>
